...
- 日期：2026-09-05
- 状态（2026-09-10）：S/E 已由 PR #159/#160 合并；I3 由 #161 合并，I4-I7 由 #162 合并（`6b7c281`）。PI 确认本地优先、远程暂缓：3.0.0 发布分支关闭远程入口，修安装默认值并补历史版本合成升级验收。R1 的远程能力、R2 的成熟度元数据统一及更广 fixtures 仍延期；R3 必须等最终 CI、tag/release、镜像 digest 与匿名拉取回读，不能以 main 合并代替。见 [本地发布验收记录](2026-09-10-core-local-release-validation.md)。
...
- I4-I7 开发基线：`536e6abfd764f18a803904631ee452815c27386f`（#161）；实现与验收见 [I 系列收口记录](2026-09-08-core-i-completion-validation.md)。
...
- 分片施工：E1a 输入/批次/截止时间/并发准入与坏行隔离已实现并通过本机验收；
